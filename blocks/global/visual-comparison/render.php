 ...
 * 它支持 `prefix` 参数，允许在不同上下文中通过 clone 字段调用。
 *
 * 🚨 避坑指南:
 * - 必须确保传入正确的 `prefix`，否则无法获取数据。
...
// 获取前缀 (Prefix Support)
// 用于区分不同模块实例中的字段 (如 sf_visual_ vs mat_visual_)
$pfx = isset($block['prefix']) ? $block['prefix'] : '';

// ==========================================================================
// II. 数据获取 (Data Retrieval)
// ==========================================================================

// 获取扁平化字段 (Flattened fields)
$title      = get_field($pfx . 'visual_title');
$raw_img_id = get_field($pfx . 'visual_before_image');
$fin_img_id = get_field($pfx . 'visual_after_image');
$phases     = get_field($pfx . 'visual_phases');
$cta_link   = get_field($pfx . 'visual_cta_link');

// ==========================================================================
// III. 数据预处理 (Data Preprocessing)
// ==========================================================================

// 1. 内容回退 (Content Fallbacks)
if ( ! $title ) {
    $title = 'Surface <br> <span class="text-primary">Transformation</span>';
}
            ...
            <div class="lg:col-span-7 w-full">
                ...
                <div x-data="{ position: 50 }" 
                    ...
                    <div class="absolute inset-0 w-full h-full">
                        <img loading="lazy" src="<?php echo esc_url($fin_url); ?>" class="w-full h-full object-cover" sizes="(min-width: 1024px) 800px, 100vw"> 
                    ...
                    <div class="absolute inset-0 w-full h-full z-10"
                        ...
                        <img loading="lazy" src="<?php echo esc_url($raw_url); ?>" class="w-full h-full object-cover" sizes="(min-width: 1024px) 800px, 100vw"> 
